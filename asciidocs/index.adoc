...
Leon Leeb - Android - Mitschrift 18.1.24
----------------------------------------

Dependency injection:
~~~~~~~~~~~~~~~~~~~~~
"new" ist immer flasch, dann kriegen wir den kontext nicht mit
wir verlassen uns auf unseren container, der macht das für uns wir sagen nur @Applicationscoped und er erstellt das object wenns das erste mal injected wird, wenn vorhanden gibt er uns das vorhandene object
...
wenn man eine neue klasse erstellt kann man im manifest diese klasse angeben, dann fängt er mit der an und nicht mit der standard application
- (mit 'android:name="ac.htl.className"')
...
- wenn wir dann properties in der singleton kallse machen und in einer anderen ein @Inject machen und ein object mit der klasse anlegen können wir auf die properties zugreifen, wichtig in der singleton klassen muss auch ein @inject über dem konstruktor stehen und es muss @AndroidEnteryPoint über der klasse wo man dependency injecten will stehen
...
~~~~~~~~~~~~~~~~~~~~~
Bei einer shallow copy werden einfach nur die obersten daten kopiert, bei einer deep kopie auch alles was dranhängt, (das oberste ist meinstens ein pointer auf das object)

Observabel:
~~~~~~~~~~~
Obervable ist eine Achitektur wo jedes mal die Liste der zu observierenden Daten durchlaufen wird wenn etwas dazu kommt
...
~~~~~~~~~~~~~~~~~~~~~~~~
Da das model nur read only ist erstllen wir keine klasse sondern einen record

In dem singleton machen wir dann ein "BehaviorSubject<Model>"

Was ist ein behaviorSubject?
^^^^^^^^^^^^^^^^^^^^^^^^^^^^
Unsere models gehen durch das behaviorSubject, es beobachtet die models und gibt das letzte aus wenn sich etwas ändert
